...
Implementation of the Ralph Wiggum technique for iterative, self-referential AI development loops in CodeBuddy Code.
...
This plugin implements Ralph using a **Stop hook** that intercepts CodeBuddy's exit attempts:

```bash
# You run ONCE:
/ralph-loop "Your task description" --completion-promise "DONE"

# Then CodeBuddy Code automatically:
# 1. Works on the task
# 2. Tries to exit
...
- CodeBuddy's previous work persists in files
- Each iteration sees modified files and git history
- CodeBuddy autonomously improves by reading its own past work in files
...
Run `/help` in CodeBuddy Code for detailed command reference and examples.
