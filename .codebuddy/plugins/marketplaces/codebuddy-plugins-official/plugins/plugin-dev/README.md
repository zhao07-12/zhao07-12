...
A comprehensive toolkit for developing CodeBuddy Code plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality CodeBuddy Code plugins:
...
4. **Plugin Settings** - Configuration patterns using .codebuddy/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in CodeBuddy Code
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${CODEBUDDY_PLUGIN_ROOT}", "block dangerous commands"
...
- ${CODEBUDDY_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${CODEBUDDY_PLUGIN_ROOT}, user vars)
...
- ${CODEBUDDY_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .codebuddy/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using CodeBuddy Code's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from CodeBuddy Code), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for CodeBuddy Code plugins
...
Install from codebuddy-marketplace:
...
This keeps CodeBuddy Code's context focused while providing deep knowledge when needed.
...
- Based on official CodeBuddy Code documentation
...
✅ **Portability**
- Use ${CODEBUDDY_PLUGIN_ROOT} everywhere
- Relative paths only
...
- Use debug mode (`codebuddy --debug`)
...
This plugin is part of the codebuddy-marketplace. To contribute improvements:
