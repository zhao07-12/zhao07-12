...
Comprehensive guidance on creating CodeBuddy Code slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${CODEBUDDY_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${CODEBUDDY_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
CodeBuddy Code activates this skill when users:
...
CodeBuddy loads references and examples as needed based on task.
...
- **Project**: `.codebuddy/commands/` (shared with team)
- **Personal**: `~/.codebuddy/commands/` (your commands)
...
- ✓ Plugin command patterns (${CODEBUDDY_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current CodeBuddy Code
