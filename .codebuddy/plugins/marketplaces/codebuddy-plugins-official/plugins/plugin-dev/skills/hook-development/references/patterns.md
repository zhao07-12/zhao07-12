...
This reference provides common, proven patterns for implementing CodeBuddy Code hooks. Use these patterns as starting points for typical hook use cases.
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CODEBUDDY_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CODEBUDDY_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$CODEBUDDY_PROJECT_DIR/.enable-security-scan"
...
**Note:** Must restart CodeBuddy Code after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$CODEBUDDY_PROJECT_DIR/.codebuddy/my-plugin.local.json"
...
**Configuration file (.codebuddy/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
