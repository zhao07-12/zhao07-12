...
This analyzes your request and creates `.codebuddy/hookify.warn-rm.local.md`.

### 2. Test It Immediately

**No restart needed!** Rules take effect on the very next tool use.

Ask CodeBuddy to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
- **`stop`**: Triggers when CodeBuddy wants to stop (for completion checks)
...
**This rule blocks the operation** - CodeBuddy will not be allowed to execute these commands.
...
**This rule warns but allows** - CodeBuddy sees the message but can still proceed.
...
**This blocks CodeBuddy from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
This plugin is part of the CodeBuddy Code Marketplace. It should be auto-discovered when the marketplace is installed.
...
**Rule not triggering:**
1. Check rule file exists in `.codebuddy/` directory (in project root, not plugin directory)
