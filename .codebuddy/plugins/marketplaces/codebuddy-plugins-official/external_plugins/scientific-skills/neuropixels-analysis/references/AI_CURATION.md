...
When using this skill within CodeBuddy Code, CodeBuddy can directly analyze waveform plots without requiring API setup. Simply:

1. Generate a unit report or plot
2. Ask CodeBuddy to analyze the visualization
3. CodeBuddy will provide expert-level curation decisions

Example workflow in CodeBuddy Code:
```python
# Generate plots for a unit
npa.plot_unit_summary(analyzer, unit_id=0, output='unit_0_summary.png')

# Then ask CodeBuddy: "Please analyze this unit's waveforms and autocorrelogram
# to determine if it's a well-isolated single unit, multi-unit activity, or noise"
```

CodeBuddy can assess:
