...
Outputs a JSON file that CodeBuddy Code can use to make informed decisions about
...
def detect_all_resources(output_path: str = None) -> Dict[str, Any]:
    ...
    Args:
        output_path: Optional path to save JSON. Defaults to .codebuddy_resources.json in cwd.

    Returns:
        Dictionary containing all resource information.
    """
    if output_path is None:
        output_path = os.path.join(os.getcwd(), ".codebuddy_resources.json")
...
def main():
    ...
    parser.add_argument(
        "-o", "--output",
        default=".codebuddy_resources.json",
        help="Output JSON file path (default: .codebuddy_resources.json)"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
