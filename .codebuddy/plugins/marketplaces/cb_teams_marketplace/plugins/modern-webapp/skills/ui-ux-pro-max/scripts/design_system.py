...
DEFAULT_STYLE = "Minimalism"

# Pre-delivery checklist items
...
class DesignSystemGenerator:
    ...
    def _apply_reasoning(self, category: str, search_results: dict) -> dict:
        ...
        if not rule:
            return {
                ...
                "style_priority": [DEFAULT_STYLE, "Flat Design"],
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        return {
            ...
            "style": {
                "name": best_style.get("Style Category", DEFAULT_STYLE),
            ...
            "colors": {
                "primary": best_color.get("Primary (Hex)", DEFAULT_COLORS["primary"]),
                "secondary": best_color.get("Secondary (Hex)", DEFAULT_COLORS["secondary"]),
                "cta": best_color.get("CTA (Hex)", DEFAULT_COLORS["cta"]),
                "background": best_color.get("Background (Hex)", DEFAULT_COLORS["background"]),
                "text": best_color.get("Text (Hex)", DEFAULT_COLORS["text"]),
                "notes": best_color.get("Notes", "")
            },
            "typography": {
                "heading": best_typography.get("Heading Font", DEFAULT_FONTS["heading"]),
                "body": best_typography.get("Body Font", DEFAULT_FONTS["body"]),
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("|------|-----|--------------|")
    lines.append(f"| Primary | `{colors.get('primary', DEFAULT_COLORS['primary'])}` | `--color-primary` |")
    lines.append(f"| Secondary | `{colors.get('secondary', DEFAULT_COLORS['secondary'])}` | `--color-secondary` |")
    lines.append(f"| CTA/Accent | `{colors.get('cta', DEFAULT_COLORS['cta'])}` | `--color-cta` |")
    lines.append(f"| Background | `{colors.get('background', DEFAULT_COLORS['background'])}` | `--color-background` |")
    lines.append(f"| Text | `{colors.get('text', DEFAULT_COLORS['text'])}` | `--color-text` |")
    lines.append("")
    if colors.get("notes"):
    ...
    lines.append("### Typography")
    lines.append("")
    lines.append(f"- **Heading Font:** {typography.get('heading', DEFAULT_FONTS['heading'])}")
    lines.append(f"- **Body Font:** {typography.get('body', DEFAULT_FONTS['body'])}")
    if typography.get("mood"):
        lines.append(f"- **Mood:** {typography.get('mood', '')}")
    if typography.get("google_fonts_url"):
    ...
    lines.append(f"  background: {colors.get('cta', DEFAULT_COLORS['cta'])};")
    ...
    lines.append(f"  color: {colors.get('primary', DEFAULT_COLORS['primary'])};")
    lines.append(f"  border: 2px solid {colors.get('primary', DEFAULT_COLORS['primary'])};")
    lines.append("  padding: 12px 24px;")
    lines.append("  border-radius: 8px;")
    lines.append("  font-weight: 600;")
    lines.append("  transition: all 200ms ease;")
    ...
    lines.append("```css")
    lines.append(".card {")
    lines.append(f"  background: {colors.get('background', DEFAULT_COLORS['background'])};")
    ...
    lines.append(f"  border-color: {colors.get('primary', DEFAULT_COLORS['primary'])};")
    lines.append("  outline: none;")
    lines.append(f"  box-shadow: 0 0 0 3px {colors.get('primary', DEFAULT_COLORS['primary'])}20;")
    lines.append("}")
    lines.append("```")
    lines.append("")
    ...
    lines.append("")
    lines.append(f"**Style:** {style.get('name', DEFAULT_STYLE)}")
    lines.append("")
    if style.get("keywords"):
        lines.append(f"**Keywords:** {style.get('keywords', '')}")
        lines.append("")
